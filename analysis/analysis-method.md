...
> 아래 $\lambda$, $\mu_s$, $\rho$ 기호는 큐잉 이론(Queueing Theory) 맥락의 성능 지표입니다.
> 이 섹션에서 사용되는 $\mu_s$(서비스율)는 위 섹션들의 $\mu_{total}$(지연 시간 평균)과 다른 의미입니다.
...
- Utilization, 이용률
- 시스템이 전체 체력($\mu_s$) 대비 현재 얼마나 부하($\lambda$)를 받고 있는지를 나타내는 비율
