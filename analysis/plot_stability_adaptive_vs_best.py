...
def save_adaptive_vs_best_stability_chart(df, target_column, title, output_filepath):
    """
    4개의 락 중 편차가 가장 작은(Best) 수치를 추출하여 Adaptive Lock과 1:1 비교하는 막대 그래프
    """
    ...
    for v in vusers:
        # 1. 4개 락 중 1위 찾기 (💡 편차는 가장 '낮은' 값이 1위이므로 idxmin 사용)
    ...
    # 막대 색상: Best 경쟁자(진한 회색), Adaptive(보라색)
    color_best = '#7f8c8d'
    color_adapt = '#9b59b6'

    # 막대 생성
    rects1 = ax.bar(x_base - width / 2, best_vals, width, color=color_best, label='Best of Others (Most Stable)')
    ...
    ax.set_ylabel(f'{target_column} Range (ms)', fontsize=12, labelpad=10)
    ...
    plt.close()
    print(f"✅ VS 안정성 비교 그래프 저장 완료: {output_filepath}")
